...
export default function VehiclesChart() {
  ...
  const [eTags, setETags] = useState(0);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setVehicles(data?.totalVehicles || 0);
        setETags(data?.totalETags || 0);
      } catch (err) {
  ...
  const chartData = [
    ...
    { name: "E-Tags", value: eTags },
  ];

  const total = vehicles + eTags;
